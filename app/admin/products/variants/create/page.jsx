...
const AddVariant = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-[#2f2f2f] text-gray-100 rounded shadow-2xl">
      <div className="flex items-center justify-between gap-3 mb-8 border-b border-gray-700 pb-4">
        <div className="text-xl flex gap-2 items-center font-semibold">
          <Box className="text-blue-400" />
          <Link href={`/admin/products/${productId}`}>Product</Link>{" "}
          <ChevronRightIcon /> <p className="text-gray-400">Add Variant</p>
        </div>
        <BackButton />
      </div>
        ...
        <div className="bg-[#383838] p-6 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <input
              type="number"
              {...register("costPrice")}
              className="w-full p-3 bg-[#1e1e1e] rounded"
            />
          </div>
          <div>
          ...
          {fields.map((item, index) => (
            ...
            >
              <input
                ...
                className="flex-1 p-2 bg-[#1e1e1e] rounded text-sm"
              />
              <input
                {...register(`attributes.${index}.value`)}
                placeholder="Value (Leather)"
                className="flex-1 p-2 bg-[#1e1e1e] rounded text-sm"
              />
              <button
                type="button"
          ...
          <div className="flex flex-wrap gap-4">
            {previews.map((url, i) => (
              <img
                key={i}
                src={url}
                className="w-24 h-24 object-cover rounded bg-[#1e1e1e]"
                alt="preview"
              />
            ))}
