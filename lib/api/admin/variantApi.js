...
export const deleteVariantImageMutation = () => {
  return useMutation({
    mutationFn: async ({ variantId, publicId }) => {
      const { data } = await api.delete(`/variants/${variantId}/image?publicId=${publicId}`);
      return data;
    },
  });
};

export const deleteVariantMutation = () => {
  return useMutation({
